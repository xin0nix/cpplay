...
pub const Row = struct {
    ...
    pub fn init(id: i32, name: []const u8, email: []const u8) Row {
        var insert = Row{
            .id = 0,
            .username = undefined,
            .email = undefined,
...
test "serialize Row" {
    ...
    }
    {
        ...
        const view = serialized[4 + 8 .. 4 + 8 + expected.len];
        try std.testing.expect(std.mem.eql(u8, view, expected));
    }
...
test "comptime constants check" {
    ...
    try std.testing.expect(ROWS_PER_PAGE == 15);
    try std.testing.expect(TABLE_MAX_ROWS == 150);
}

...
TEST(MinDiffTest, Leet1) {
  vector nums{90};
  EXPECT_EQ(BruteForce().minimumDifference(nums, 1), 0);
}

TEST(MinDiffTest, Leet2) {
  vector nums{9, 4, 1, 7};
  EXPECT_EQ(BruteForce().minimumDifference(nums, 2), 2);
}

TEST(MinDiffTest, Custom1) {
  vector nums{2, 8, 9, 6, 2, 0, 5};
  EXPECT_EQ(BruteForce().minimumDifference(nums, 4), 4);
}

TEST(MinDiffTest, Custom2) {
  vector nums{2, 8, 9, 6, 2, 0, 5};
  EXPECT_EQ(BruteForce().minimumDifference(nums, 2), 0);
}

TEST(MinDiffTest, Custom3) {
  vector nums{2, 8, 9, 6, 2, 0, 5};
  EXPECT_EQ(BruteForce().minimumDifference(nums, 3), 2);
}

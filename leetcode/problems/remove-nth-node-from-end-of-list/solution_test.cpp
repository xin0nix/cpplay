...
TEST(RemoveNthTest, LeetCode1) {
  ...
  ListNode *h = BruteForce().removeNthFromEnd(lw.getHead(), 2);
  auto res = buildVec(h);
...
TEST(RemoveNthTest, LeetCode2) {
  ListWrapper lw({1});
  ListNode *h = BruteForce().removeNthFromEnd(lw.getHead(), 1);
  auto res = buildVec(h);
...
TEST(RemoveNthTest, RemoveHead) {
  ...
  ListNode *h = BruteForce().removeNthFromEnd(lw.getHead(), 3);
  auto res = buildVec(h);

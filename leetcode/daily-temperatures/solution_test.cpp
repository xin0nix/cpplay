...
TEST(DailyTemperaturesTest, Empty) {
  ...
  EXPECT_THAT(Solution().dailyTemperatures(temperatures), ElementsAre());
}

TEST(DailyTemperaturesTest, Basic1) {
  vector<int> temperatures{73, 74, 75, 71, 69, 72, 76, 73};
  EXPECT_THAT(Solution().dailyTemperatures(temperatures),
...
TEST(DailyTemperaturesTest, Basic3) {
  ...
  EXPECT_THAT(Solution().dailyTemperatures(temperatures), ElementsAre(0, 0, 0));
}
